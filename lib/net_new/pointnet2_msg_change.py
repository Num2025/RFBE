...
class Atten_Fusion_Conv(nn.Module):
    def __init__(self, inplanes_I, inplanes_P, outplanes):
        ...
        self.IA_Layer = IA_Layer(channels=[inplanes_I, inplanes_P])#ILFUSION
...
class Pointnet2MSG(nn.Module):
    def __init__(self, input_channels=6, use_xyz=True):
        ...
        #     # print("input3d:", input3d)
        #     self.clfusion.append(CLFM(input2d, input3d))
        # CLF1_LIST = [[3, 128], [64, 256], [128, 512], [256, 512]]
        # self.clfusiondeconv = nn.ModuleList()
        # for (input2d, input3d) in CLF1_LIST:
        #     # print("input2d:", input2d)
        #     # print("input3d:", input3d)
        #     self.clfusiondeconv.append(CLFM(input2d, input3d))
        # CLFF_LIST = [[64,128]]
        # self.clfusionend = nn.ModuleList()
        # for (input2d, input3d) in CLFF_LIST:
        #     # print("input2d:", input2d)
        #     # print("input3d:", input3d)
        #     self.clfusionend.append(CLFM(input2d, input3d))
        ...
        for k in range(cfg.RPN.FP_MLPS.__len__()):
            ...
            # print("FP_MODULES:", str(pre_channel), "__" + str(skip_channel_list[k]), "__", cfg.RPN.FP_MLPS[k])
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, image=None, xy=None):
        ...
        # print("Pointnet2_msg_details:_________")
        # print("l_xyz data size", len(l_xyz))
        # print("l_features data size", len(l_features))
        ...
        for i in range(len(self.SA_modules)):
            li_xyz, li_features, li_index = self.SA_modules[i](l_xyz[i], l_features[i])  # 输入点云特征模块

            if cfg.LI_FUSION.ENABLED:
                li_index = li_index.long().unsqueeze(-1).repeat(1, 1, 2)
                li_xy_cor = torch.gather(l_xy_cor[i], 1, li_index)  # 只保留有特征的xy？
                ...
                print("____",img_gather_feature.size())
                li_features = self.Fusion_Conv[i](li_features, img_gather_feature)  # LIFUSION
        ...
        # print("SA_MODUOLES_END")

        for i in range(-1, -(len(self.FP_modules) + 1), -1):  # 需要仔细看
            l_features[i - 1] = self.FP_modules[i](
                l_xyz[i - 1], l_xyz[i], l_features[i - 1], l_features[i]
            )
            # _, l_features[i-1] = self.clfusiondeconv[i](l_xy_cor[i-1].permute(0,2,1), img[i-1], l_features[i-1])

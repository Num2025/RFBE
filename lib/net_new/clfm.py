...
from lib.net.utils import grid_sample_wrapper, mesh_grid, k_nearest_neighbor, batch_indexing, softmax, timer
from lib.net.mlp import Conv1dNormRelu, Conv2dNormRelu
...
class FusionAwareInterp(nn.Module):
    ...
    def forward(self, uv, feat_2d, feat_3d):
        ...
        knn_offset_squared=knn_offset ** 2
        sum_of_squares = torch.sum(knn_offset_squared, dim=1, keepdim=True)
        knn_offset_norm = torch.sqrt(sum_of_squares)
...
class FusionAwareInterpp2i(nn.Module):
    def __init__(self, n_channels_2d, k=3, norm=None):
        super().__init__()
        self.k = k
        # self.out_conv = Conv2dNormRelu(n_channels_2d, n_channels_2d, norm=norm)
        self.score_net = nn.Sequential(
            Conv2dNormRelu(3, 16),  # [dx, dy, |dx, dy|_2, sim]
            Conv2dNormRelu(16, n_channels_2d, act='sigmoid'),
        )

    def forward(self, uv, feat_2d, feat_3d):
        bs, n_channels_2d, image_h, image_w = feat_2d.shape
        n_channels_3d = feat_3d.shape[1]
        ...
        score_input = torch.cat([knn_offset, knn_offset_norm], dim=1)  # [B, 4, HW, K]
        score = self.score_net(score_input)  # [B, n_channels_2d, HW, k]
        # score = softmax(score, dim=-1)  # [B, n_channels_3d, HW, k]
        score = score.sum(dim=-1)
        att = score.reshape([bs,n_channels_2d,image_h,image_w])
        final = att * feat_2d
        # final = score * knn_feat3d  # [B, n_channels_3d, HW, k]
        # final = final.sum(dim=-1).reshape(bs, -1, image_h, image_w)  # [B, n_channels_3d, H, W]
        # final = self.out_conv(final)
...
class FusionAwareInterpii(nn.Module):
    def __init__(self, n_channels_2d,n_channels_3d, k=1, norm=None):
        super().__init__()
        self.k = k
        self.out_conv = Conv2dNormRelu(n_channels_3d, n_channels_2d, norm=norm)
        self.score_net = nn.Sequential(
            Conv2dNormRelu(3, 16),  # [dx, dy, |dx, dy|_2, sim]
            Conv2dNormRelu(16, n_channels_3d, act='sigmoid'),
        )
    ...
    def forward(self, uv, feat_2d, feat_3d):
        ...
        final = score * knn_feat3d  # [B, n_channels_3d, HW, k]
        final = final.sum(dim=-1).reshape(bs, -1, image_h, image_w)  # [B, n_channels_3d, H, W]
        final = self.out_conv(final)# [B, n_channels_2d, H, W]

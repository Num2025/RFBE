...
def model_joint_fn_decorator():
    ...
    def get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict, img_seg_predict=None,
                     rpn_image_seg_label=None, img_seg_sparse_predict=None):
        ...
        if cfg.RPN.LOSS_CLS == 'DiceLoss':
            ...
            if cfg.USE_MC_LOSS:
                ...
                diff = torch.abs(p1_score - p2_score)
                threshold = 0.35  # 差值阈值，例如 0.3 或 0.5

                # 根据差值动态计算权重
                weight_A = p1_score / (p1_score + p2_score + 1e-6)
                weight_B = p2_score / (p1_score + p2_score + 1e-6)
                # 动态调整权重，当差值大于等于阈值时，使用 A 和 B 的权重，否则使用默认的 0.5
                i2p_weight = torch.where(diff >= threshold, weight_A, torch.full_like(weight_A, 0.5))
                p2i_weight = torch.where(diff >= threshold, weight_B, torch.full_like(weight_B, 0.5))
                mc_loss = i2p_weight * kl_loss_i2p + p2i_weight * kl_loss_p2i
                if cfg.ADD_MC_MASK:

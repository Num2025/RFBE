...
torch::Tensor k_nearest_neighbor_cuda(torch::Tensor input_xyz, torch::Tensor query_xyz, int k) {
	...
	if (n_dim == 2)
		k_nearest_neighbor_2d_kernel_wrapper(batch_size, n_queries, n_inputs, k, reinterpret_cast<float*>(query_xyz.data_ptr()), reinterpret_cast<float*>(input_xyz.data_ptr()), reinterpret_cast<int64_t*>(indices.data_ptr()));
	else
		k_nearest_neighbor_3d_kernel_wrapper(batch_size, n_queries, n_inputs, k, reinterpret_cast<float*>(query_xyz.data_ptr()), reinterpret_cast<float*>(input_xyz.data_ptr()), reinterpret_cast<int64_t*>(indices.data_ptr()));

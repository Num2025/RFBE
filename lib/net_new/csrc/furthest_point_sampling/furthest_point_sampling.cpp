...
torch::Tensor furthest_point_sampling_cuda(torch::Tensor points_xyz, const int n_samples) {
	...
	furthest_point_sampling_kernel_wrapper(reinterpret_cast<float*>(points_xyz.data_ptr()), reinterpret_cast<float*>(dists_temp.data_ptr()), batch_size, n_points, n_samples, reinterpret_cast<int64_t*>(furthest_indices.data_ptr()));

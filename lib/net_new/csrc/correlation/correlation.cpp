...
torch::Tensor correlation_forward_cuda(torch::Tensor& input1, torch::Tensor& input2, int max_displacement) {
	...
	correlation_forward_kernel_wrapper(reinterpret_cast<float*>(output.data_ptr()), reinterpret_cast<float*>(input1.data_ptr()), reinterpret_cast<float*>(input2.data_ptr()),
...
std::pair<torch::Tensor, torch::Tensor> correlation_backward_cuda(torch::Tensor& grad_output, torch::Tensor& input1, torch::Tensor& input2, int max_displacement) {
	...
	correlation_backward_kernel_wrapper(reinterpret_cast<float*>(grad_output.data_ptr()),
										reinterpret_cast<float*>(grad_input1.data_ptr()), reinterpret_cast<float*>(grad_input2.data_ptr()),
										reinterpret_cast<float*>(input1.data_ptr()), reinterpret_cast<float*>(input2.data_ptr()),
										batch_size, in_channels, height, width, max_displacement);

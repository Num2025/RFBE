...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_rpn_with_li_fusion(self, index):
        ...
        if sample_id < 10000:
            ...
            # print(sample_id)
            # print("pts_lidar",len(pts_lidar))
            # get valid point (projected points should be in image)
            pts_rect = calib.lidar_to_rect(pts_lidar[:, 0:3])
            # print("pts_rect", len(pts_rect))
        ...
        if cfg.INPUT_CROSS_FUSION: ## shape: 384, 1280, 4
            point2img = np.zeros([720, 1280, 4], dtype=np.float)
        ...
        if cfg.USE_IMAGE_LOSS:
            rpn_image_seg_label = np.zeros([720, 1280, 1], dtype=np.float)
            rpn_image_seg_label[ret_pts_origin_xy[:, 1].astype(int), ret_pts_origin_xy[:, 0].astype(int), 0] = rpn_cls_label
            sample_info['img_seg_mask'] = rpn_image_seg_label
        return sample_info
